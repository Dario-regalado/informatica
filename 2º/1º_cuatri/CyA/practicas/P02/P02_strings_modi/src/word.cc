...
/**
 * @brief hace la potencia de una cadena
 * 
 * @param potencia
 * @return Word
 */
Word Word::Potencia(const int potencia) {
  Word secuencia;
  if (potencia != 0)
    for (int i = 1; i <= potencia; i++) {
      secuencia = secuencia + Word(word_);
    }
  else {
    secuencia.word_.clear();
    secuencia.word_ = "&";
  }
  return Word(secuencia);
}
...
/**
 * @brief suma dos cadenas
 * 
 * @param primera_cadena
 * @param segunda_cadena
 * @return Word
 */
Word operator+(const Word& primera_cadena, const Word& segunda_cadena) {
  return primera_cadena.GetWord() + segunda_cadena.GetWord();
}