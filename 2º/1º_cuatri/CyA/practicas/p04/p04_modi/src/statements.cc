/**
  ...
  * @date 08/10/2024
  * @brief implementation of the Statements class 
  * @Practica 04_regex
 ...
 * @param input_expre 
 */
Statements::Statements(const std::regex& input_expre) : expression_{R"(^\s+(switch).*\(\s*(.*)\s*\)|\b(for|while)\b)", std::regex::ECMAScript|std::regex::multiline} {
...
void Statements::EvaluateFile(const std::string& file_input) {
  for (std::sregex_iterator it{file_input.begin(), file_input.end(), expression_}; it != std::sregex_iterator(); ++it) {
    ...
    if(std::regex_search(match.str(), std::regex("for|while"))) {
      ...
      types_.emplace_back("CONDITIONAL");
      if(std::regex_search(match.str(), std::regex(R"(^\s+(switch).*\(\s*(.*)\s*\))"))) {
        variable_names_.emplace_back("switch: " + match.str(2));
      } else {
        variable_names_.emplace_back("if");
      }
    }
  }
}
...
std::ostream& operator<<(std::ostream& output, const Statements& variable_salida) {
  ...
  int fors{0}, whiles{0}, conditionals{0}; 
  ...
  output << "conditionals: " << conditionals << std::endl;
