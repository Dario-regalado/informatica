...
template <class T> class dll_t {
 public:
  ...
  //modificacion
  void InsertAfter(dll_node_t<T>*, dll_node_t<T>*);
  void duplicate();
  void ImmparEraser();
...
template <class T>
void dll_t<T>::InsertAfter(dll_node_t<T>* prev, dll_node_t<T>* next) {
...
template <class T>
void dll_t<T>::ImmparEraser() {
  dll_node_t<T>* aux = head_;
  while (aux != NULL) {
    delete erase(aux);
    aux = aux->get_next() != NULL ? aux->get_next()->get_next() : aux->get_next();
  }
